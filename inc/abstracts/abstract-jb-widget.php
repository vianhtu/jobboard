 ...
 * @author   JobBoard
 * @category Widgets
 * @package  JobBoard/Abstracts
 * @version  1.0.0
 * @extends  WP_Widget
 */
abstract class JB_Widget extends WP_Widget {
			...
			switch ( $setting['type'] ) {
				...
				case 'radio':
					...
					<p>
						...
						<div>
						<?php foreach ($setting['options'] as $option => $label): ?>
							<input class="radio <?php echo esc_attr( $class ); ?>" id="<?php echo esc_attr( $this->get_field_id( $key ) . '-' . $option ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" type="radio" value="<?php echo esc_attr($option); ?>" <?php checked( $value, $option ); ?> />
							<label for="<?php echo $this->get_field_id( $key ) . '-' . $option; ?>"><?php echo $label; ?></label>
						<?php endforeach; ?>
						</div>
					</p>
					<?php
